...
const Page = () => {
  ...
  const sectionToDisplay= schoolss?.filter((s)=>s.id==parseInt(formik.values.searchId))
   ...
   let componentToRender;
    ...
    if (teacherView?.length > 0 && parseInt(formik.values.semester) === 1) {
        componentToRender =   <FindStudent
        teacherView={teacherView}
       
      />
    } else if (teacherView?.length > 0 && parseInt(formik.values.semester) === 2) {
      componentToRender =   <ForSemesterTwo
      teacherView={teacherView}
     
    />
    } else if (teacherView?.length > 0 && parseInt(formik.values.semester) === 3) {
      componentToRender =   <ForAll
      teacherView={teacherView}
     
    />
    } else {
        componentToRender = <div className="flex flex-col justify-center items-center gap-3 lg:mt-16">
      ...
      </div> // Render the default component if none of the conditions match
    }
  ...
  return (
   ...
   <form onSubmit={formik.handleSubmit} className="w-full flex flex-col  justify-center items-center">
   ...
   <div className=" px-4">
         ...
         </svg>  Generating Rank ...</>:"Generate Rank"}
              </button>
